    ...
    >
      ...
      <div class="flex-1 overflow-y-auto px-4 py-5">
        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950/40">
          <p class="text-[9px] font-black uppercase tracking-[0.16em] text-slate-400">{{ $t('workspace_layout.workshop') }}</p>
        ...
        <div class="mt-7 grid grid-cols-2 gap-2">
          <div class="rounded-xl bg-slate-100 p-3 dark:bg-slate-800">
            <p class="text-[9px] font-bold text-slate-400">{{ $t('workspace_layout.files') }}</p>
            <p class="mt-1 text-xs font-black" :class="$can('factory.download') ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-400'">{{ $can('factory.download') ? $t('workspace_layout.allowed') : $t('workspace_layout.blocked') }}</p>
          </div>
          <div class="rounded-xl bg-slate-100 p-3 dark:bg-slate-800">
            <p class="text-[9px] font-bold text-slate-400">{{ $t('workspace_layout.update') }}</p>
            <p class="mt-1 text-xs font-black" :class="$can('factory.order_update') ? 'text-emerald-600 dark:text-emerald-300' : 'text-slate-400'">{{ $can('factory.order_update') ? $t('workspace_layout.allowed') : $t('workspace_layout.blocked') }}</p>
          </div>
        </div>
      </div>
    ...
    <div class="min-h-screen lg:pl-72">
      <header class="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur dark:border-slate-800 dark:bg-slate-900/90 sm:px-6 lg:px-8">
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0">
            <p class="truncate text-sm font-semibold text-slate-900 dark:text-white">{{ factoryLabel }}</p>
...
export default {
  ...
  computed: {
    ...
    factoryLabel() { return this.currentUser?.factory?.name || this.$t('workspace_layout.workshop') },
