    ...
    >
      <div class="w-full h-full max-w-md">
        ...
        >
          <div
            v-if="dxfUrl"
            class="relative h-full w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900"
          >
            <!-- FileViewer-ը թող աշխատի DXF/PDF–ի հետ -->
            <FileViewer :key="dxfUrl" :dxf-url="dxfUrl" />

            <button
              class="absolute right-3 top-3 rounded-full bg-black/50 px-2 py-1 text-[10px] font-semibold text-white hover:bg-black/70"
              @click="$emit('close-dxf')"
            >
              Փակել դիտումը
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    isDXF(path) {
      return path?.toLowerCase().endsWith('.dxf') || false
    },
    ...
    statusLabel(status) {
      switch (status) {
        ...
        case 'canceled':
          return 'Մերժված'
        case 'date_changed':
          return 'Ժամկետը փոփոխված է'
        case 'finished':
          return 'Ավարտված'
        default:
          return 'Առանց կարգավիճակի'
      }
    },
