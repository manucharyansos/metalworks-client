<template>
  <section class="flex h-full flex-col overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
    ...
    <div class="flex-1 p-4">
        ...
        <div v-for="(extension, index) in extensions" :key="extension.id" class="rounded-2xl border border-slate-100 bg-slate-50/70 p-2.5 transition hover:border-slate-200 dark:border-slate-800 dark:bg-slate-950/40 dark:hover:border-slate-700">
          <div v-if="editIndex !== index" class="flex items-center justify-between gap-3">
            ...
            <div class="flex shrink-0 items-center gap-1">
              <button type="button" class="rounded-xl p-2 text-slate-400 transition hover:bg-white hover:text-slate-800 dark:hover:bg-slate-800 dark:hover:text-white" aria-label="Խմբագրել ֆայլի տեսակը" @click="startEditing(index)">
              ...
              <button type="button" class="rounded-xl p-2 text-slate-400 transition hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-950/30 dark:hover:text-rose-300" aria-label="Ջնջել ֆայլի տեսակը" @click="deleteExtension(extension.id)">
          ...
          <div v-else class="flex items-center gap-2">
            ...
            <button type="button" class="rounded-xl bg-slate-950 p-2 text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-950" aria-label="Պահպանել ֆայլի տեսակը" @click="saveEdit(extension.id)">
            ...
            <button type="button" class="rounded-xl border border-slate-200 p-2 text-slate-500 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800" aria-label="Չեղարկել խմբագրումը" @click="cancelEdit">
      ...
      <div v-else class="flex min-h-[180px] items-center justify-center rounded-2xl border border-dashed border-slate-200 text-center text-xs text-slate-400 dark:border-slate-800">Դեռ ֆայլի տեսակ ավելացված չէ</div>
    </div>
