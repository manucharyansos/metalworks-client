  ...
  <main class="p-4 flex flex-col items-start gap-1 relative">
    ...
    <div class="flex flex-row items-start gap-4">
        ...
        >
          <div class="flex flex-col gap-2">
            <div v-for="factory in getFactory" :key="factory.id">
              <button
                class="border border-gray-300 rounded-lg w-64 p-2 flex justify-between items-center"
                :class="{
              ...
              >
                ...
                <span v-if="loadingFactoryId === factory.id">
                  <svg
                    class="w-4 h-4 animate-spin inline ml-2"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                  >
              ...
              <transition name="slide-down">
                ...
                >
                  ...
                  <div class="w-64 mt-12">
                    <button
                      class="bg-green-600 rounded-lg text-white w-full py-1.5 flex justify-center items-center"
                      :disabled="loading"
      ...
      <div class="w-full">
        ...
        >
          <p class="my-6">{{ selectedFile.original_name }}</p>
          <!-- DXF Viewer -->
          <DxfViewer
            v-if="fileType === 'dxf' && dxfUrl"
            :key="dxfUrl"
            :dxf-url="dxfUrl"
            style="width: 700px"
          />
          <!-- PDF Viewer -->
          <div v-if="fileType === 'pdf'" class="pdf-section w-full">
            <embed
            ...
            />
            <a
              class="mt-2 inline-block bg-gray-700 text-white rounded-xl px-5 py-1"
          ...
          <!-- Image Viewer -->
          <div v-if="fileType === 'image'" class="image-section w-full">
          ...
          <!-- CAD File Viewer (SLD/Solid) -->
          <div v-if="fileType === 'cad'" class="cad-section w-full">
...
export default {
  ...
  created() {
    ...
    if (this.id) {
      ...
      Promise.all([this.fetchPmp(this.id), this.fetchFactory()])
        .then(([pmpResponse, factoryResponse]) => {})
        .catch((error) => {
          this.$notify({
            text: error,
            duration: 3000,
      ...
      this.$notify({
        text: 'Հաշվի ID-ն բացակայում է: Խնդրում ենք ստուգել URL-ը:',
        duration: 3000,
        speed: 1000,
        position: 'top',
        type: 'error',
  ...
  methods: {
    ...
    viewFile(filePath, file) {
      ...
      this.isDxfFile =
        file.factory_id === this.selectedFactoryId && this.isDxfFile
      this.fileType = this.getFileType(file.original_name || filePath)
    },
    async downloadFile(file) {
      if (file && typeof file.path === 'string') {
        const sanitizedPath = file.path.replace(/\\/g, '/')
        await this.downloadUploadedFile(sanitizedPath, file.original_name)
      } else {
        this.$notify({
    ...
    selectBreadcrumb(index) {
      ...
      if (index === 0) {
        ...
        if (selectedFactory) {
          ...
          this.isDxfFile = selectedFactory.value === 'DXF'
          this.selectedFiles = this.getPmp.files.filter(
