    ...
    <div v-if="$can('workers.view')" class="mx-auto max-w-[1500px] space-y-6">
      <section class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div class="flex items-center gap-2">
          <h1 class="text-2xl font-black tracking-tight text-slate-950 dark:text-white sm:text-3xl">Աշխատակիցներ</h1>
          <InfoTooltip>Աշխատակիցների տվյալներ, հաստիք և արտադրամաս։</InfoTooltip>
        </div>
        <div class="flex w-full flex-col gap-2 sm:flex-row lg:w-auto">
          <div class="relative w-full sm:w-80">
            ...
            <input v-model="searchQuery" type="text" placeholder="Անուն, էլ․ փոստ, հեռախոս..." class="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-3 pr-10 text-sm text-slate-700 outline-none transition focus:border-slate-400 focus:ring-2 focus:ring-slate-900/5 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200" />
          </div>
  ...
  methods: {
    ...
    async ensureFormOptions() {
      ...
      try {
        ...
        this.$notify?.({ type: 'error', text: e.response?.data?.message || 'Չհաջողվեց բեռնել աշխատակցի ձևի տվյալները' })
        return false
      }
    },
    async openFromQuery() {
    ...
    async openCreate() {
      if (!this.$can('workers.create') || !(await this.ensureFormOptions())) return
